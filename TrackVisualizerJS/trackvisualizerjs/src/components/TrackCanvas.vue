<template>
    <div class="d-flex">
        <div class="">
            ...
            <div v-if="slamData.length > 0">
                <input type="range" class="form-range" :max="slamData.length - 1" v-model="slamDataCurrentFrame" />
                <button :disabled="slamDataCurrentFrame == 0" class="btn btn-primary" @click="slamDataCurrentFrame--">&lt;</button>
                <button v-if="slamDataInterval == 0" class="btn btn-success" @click="playSlamData()">Play</button>
                <button v-else class="btn btn-danger" @click="pauseSlamData()">Pause</button>
                <button :disabled="slamDataCurrentFrame == slamData.length - 1" class="btn btn-primary" @click="slamDataCurrentFrame++">&gt;</button>
                Frame {{ slamDataCurrentFrame }} / {{ slamData.length - 1 }}
            </div>
            <canvas
                ref="canvas"
        ...
        <div class="ms-3" style="flex: 1">
            ...
            <div class="form-group">
                <label>draw ground truth</label>
                <input type="checkbox" class="form-check" v-model="drawGroundTruth" />
            </div>
            <div class="form-group">
                <label>slam scale</label>
                <div class="d-flex">
                    <input type="range" class="form-range w-0 flex-1" min="1" max="100" v-model.number="slamScale" />
...
export default class TrackCanvas extends Vue {
    ...
    drawCurvature(x: number, y: number, angle: number, curvature: number, d: number, color: string) {
        if (Math.abs(curvature * d) < 0.001) {
            const [x2, y2] = plus([x, y], mul(rot([1, 0], angle), d));
            this.context.beginPath();
        ...
        this.context.beginPath();
        const [x2, y2] = plus([x, y], rot([1 / curvature, 0], angle + 90));
        this.context.strokeStyle = "black";
        if (curvature > 0)
            this.context.arc(x2, y2, 1 / curvature, ((angle - 90) / 180) * Math.PI, d * curvature + ((angle - 90) / 180) * Math.PI, false);
        else this.context.arc(x2, y2, -1 / curvature, d * curvature + ((angle + 90) / 180) * Math.PI, +((angle + 90) / 180) * Math.PI, false);
    ...
    canvasMouseMove(event: MouseEvent) {
        ...
        var p = [...this.groundTruthToBeAdded.slice(-4), this.getPos(event)].reduce(
            (a, c, _, arr) => [a[0] + c[0] / arr.length, a[1] + c[1] / arr.length],
            [0, 0]
        );
        ...
        /*const messages = text
            ...
            .map((msg) => {
                ...
                return {
                    ...
                    payload: YAML.parse(m[1]),
                };
            });

        const datas = sliceIntoChunks(
            messages.map((m) => [m.topic, m.payload]),
            4
        ).map((c) => Object.fromEntries(c)) as BagData[];
    ...
    async playSlamData() {
        if (this.trackData.groundTruth.length != 0) {
            ...
            for (let i = 0; i < this.trackData.groundTruth.length; i++) {
                let closesPylonIndex = 0;
                for (let j = 0; j < this.trackData.pylons.length; j++) {
                    let pylon = [this.trackData.pylons[j].x, this.trackData.pylons[j].y] as const;
                    let closesPylon = [this.trackData.pylons[closesPylonIndex].x, this.trackData.pylons[closesPylonIndex].y] as const;
                    let targetCar = this.trackData.groundTruth[i];
                    if (d(targetCar, pylon) < d(targetCar, closesPylon)) closesPylonIndex = j;
                }
                this.slamData.push({
                    "/slam/map": null as any,
                    "/slam/car": {
                        header: null as any,
                        car_state: {
                            x: this.trackData.groundTruth[i][0] / 10,
                            y: this.trackData.groundTruth[i][1] / 10,
                            theta: angle(min(this.trackData.groundTruth[i], this.trackData.groundTruth[i + 1] ?? [0, 0])),
                        },
                    },
                    "/slam/debug/map": {
                        positions: this.trackData.pylons.slice(0, closesPylonIndex + 7).map((p) => ({ x: p.x / 10, y: p.y / 10, z: 0 })),
                        covariances: this.trackData.pylons.slice(0, closesPylonIndex + 7).map((_) => ({ covariance: [0, 0, 0, 0] })),
                        colors: this.trackData.pylons.slice(0, closesPylonIndex + 7).map((p) => (p.color == "#0000FF" ? 121 : 98)),
    ...
    async pauseSlamData() {
        ...
        this.slamDataInterval = 0;
    }
    @Watch("slamDataCurrentFrame")
    public updateFrame() {
        this.reset();
...
function curvature(ps: (readonly [number, number])[]) {
    if (ps.length < 3) return 0;
    let p = ps[0];
    let q = ps[ps.length - 1];
    let theta = angle(min(ps[1], ps[0]));
    let alpha = angle(min(q, p)) - theta;
    let s = d(p, q);
    return (2 * Math.sin((alpha * Math.PI) / 180)) / s; // s=r2*sin(beta/2)
}
